...
impl Frog {
    pub fn new() -> Self {
        ...
        debug!(target: "Frog::new", "A new Frog as been created");
        Default::default()
    }
    ...
    pub fn hop(&mut self) {
        ...
        info!(target: "Frog::hop", "A Frog hopped! It now has {} energy left", self.energy);
        if self.energy == 0 {
            // 4. Use warn!() to warn that the frog will go to sleep since he ran out of energy
            warn!(target: "Frog::hop", "The Frog will now go to sleep to replenish its energy");
            self.sleep();
        }
    }
...
impl Default for Frog {
    fn default() -> Self {
        ...
        trace!(target: "Frog::default", "A default Frog value has been generated: {:?}", frog);
        frog
    }
}
