...
fn main() {
    ...
    let child_a = thread::spawn(move || {
        rxx.iter().for_each(|x| println!("Child A consumed: {}", x));
        println!("Child A is done.");
    });

    let child_b = thread::spawn(move || {
        rxx2.iter().for_each(|x| println!("Child B consumed: {}", x));
        println!("Child B is done.");
    });

    (0..50).for_each(|x| txx.send(x).unwrap());
    drop(txx);
